...
from .base.compatibility import (cInputGenerator, decodeGenerator,
                                 encodeGenerator)
...
class thulac:
    def __init__(
        ...
        self.__idiomDict = Postprocesser((self.__prefix + "idiom.dat"), "i",
                                         False)
        ...
        if (self.__user_specified_dict_name):
            self.__userDict = Postprocesser(self.__user_specified_dict_name,
                                            "uw", True)
        if (self.__use_filter):
            self.__myfilter = Filter((self.__prefix + "xu.dat"),
                                     (self.__prefix + "time.dat"))
        if (self.__seg_only):
            self.__cws_tagging_decoder = CBTaggingDecoder()
            self.__cws_tagging_decoder.init((self.__prefix + "cws_model.bin"),
                                            (self.__prefix + "cws_dat.bin"),
            ...
            self.__tagging_decoder.init((self.__prefix + "model_c_model.bin"),
                                        (self.__prefix + "model_c_dat.bin"),
                                        (self.__prefix + "model_c_label.txt"))
        ...
        if (text):
            for line in oiraw:
                if (self.__seg_only):
                    ...
                    temp_txt = reduce(lambda x, y: x + ' ' + "".join(y),
                                      cut_method(line), '') + '\n'
                txt += temp_txt[1:]
            return txt[:-1]
        else:
            for line in oiraw:
                if (line):
                    if (self.__seg_only):
                        ...
                        array += (reduce(lambda x, y: x + [[y[0], y[2]]],
                                         cut_method(line), []))
                array += [['\n', '']]
            return array[:-1]
    ...
    def __cutline(self, oiraw):
        ...
        for oiraw in vec:
            if (self.__useT2S):
                traw, __poc_cands = self.__preprocesserpreprocesser.clean(
                    oiraw)
            ...
            if (len(raw) > 0):
                if (self.__seg_only):
                    tmp, tagged = self.__cws_tagging_decoder.segmentTag(
                        raw, __poc_cands)
                    ...
                    tmp, tagged = self.__tagging_decoder.segmentTag(
                        raw, __poc_cands)
                    if (self.__userDict is not None):
        ...
        if (self.__seg_only):
            ...
            return map(lambda x: (encode(x[0]), encode(x[1]), encode(x[2])),
                       ans)
    ...
    def __SoInit(self):
        ...
        return SoExtention(self.__prefix, self.__user_specified_dict_name,
                           self.__useT2S, self.__seg_only)
